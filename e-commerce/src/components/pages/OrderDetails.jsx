...
import ForOrderDetails from "../../Loaders/ForOrderDetails";
...
function OrderDetails() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    onSuccess: (data) => {
      setSelectedPaymentStatus(data?.orderDetails?.payment_status || "pending");
    },
  });
...
const updatePaymentMutation = useMutation({
  mutationFn: async (newStatus) => {
    const res = await api.put(
      `/api/updatePaymentStatus/${id}`,   // ✅ matches your route
  ...
  onSuccess: () => {
    setUpdateMsg("Payment status updated successfully!");
    queryClient.invalidateQueries(["orderDetails", id]);
    setTimeout(() => setUpdateMsg(""), 3000);
  },
  onError: () => {
    setUpdateMsg("Failed to update payment status.");
    setTimeout(() => setUpdateMsg(""), 3000);
  },
});
  ...
  if (isError) {
    return (
      <div className="h-screen flex justify-center items-center">
        <h1 className="text-red-500 text-xl">Failed to load order details</h1>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border">
              <div className="p-6">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  <img
                    onClick={() =>
                      navigate(`/product/${data.orderDetails.product_id}`)
                    }
                    src={imageUrl}
              ...
              {order?.order_status === "cancelled" ? (
                ...
                <div>
                  {steps.map((step, index) => (
                    <div key={step} className="flex gap-4 relative pb-10">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border p-6">
              ...
              <p className="text-gray-600 mt-2">{order?.addres}</p>
              <p className="text-gray-600">
                {order?.city}, {order?.state}
              </p>
              <p className="text-gray-600">{order?.pincode}</p>
              <p className="font-medium mt-3">{order?.mobile}</p>
            </div>
              ...
              <div className="space-y-3">
                ...
                <div className="pt-4 border-t mt-2">
                  ...
                  <button
                    ...
                    className="mt-3 w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white text-sm font-medium py-2 rounded-lg transition-colors"
                  >
                    {updatePaymentMutation.isLoading
                      ? "Updating..."
                      : "Update Status"}
