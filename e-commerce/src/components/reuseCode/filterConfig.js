export const filterConfig = {
  Men: {
    categories: ["Shirts", "T-Shirts", "Jeans", "Jackets", "Hoodies"],
    sizes: ["S", "M", "L", "XL", "2XL"],
    colors: ["Black", "White", "Blue", "Gray", "Red"],
    prices: ["500", "1000", "1500", "2000", "3000"]
  },

  Women: {
    categories: ["Tops", "Kurtis", "Jeans", "Saree", "Dress"],
    sizes: ["XS", "S", "M", "L", "XL"],
    colors: ["Pink", "Black", "Red", "White", "Blue"],
    prices: ["500", "1000", "1500", "2000", "3000"]
  },

  Mouse: {
    brand: ["Logitech", "HP", "Dell", "Razer", "Zebronics"],
    prices: ["300", "500", "700", "1000", "1500"],
    type: ["Wired", "Wireless"]
  },

  Camera: {
    brand: ["Canon", "Nikon", "Sony", "Fujifilm"],
    prices: ["20000", "30000", "40000", "50000"],
    type: ["DSLR", "Mirrorless", "Digital"]
  },

  Earphones: {
    brand: ["Boat", "Sony", "JBL", "Realme", "Boult"],
    type: ["Wired", "Wireless", "Neckband"],
    prices: ["300", "500", "1000", "1500", "2000"],
    colors: ["Black", "White", "Blue", "Red"]
  },

  Mobiles: {
    brand: ["Samsung", "Apple", "Vivo", "Oppo", "Realme", "Xiaomi"],
    prices: ["5000", "10000", "15000", "20000", "30000"],
    ram: ["4GB", "6GB", "8GB", "12GB"],
    storage: ["64GB", "128GB", "256GB"]
  },

  Speakers: {
    brand: ["Boat", "JBL", "Sony", "Zebronics", "Marshall"],
    prices: ["500", "800", "1000", "1500", "2000"],
    type: ["Bluetooth", "Wired", "Portable"]
  },

  Televisions: {
    brand: ["Sony", "LG", "Samsung", "Mi", "OnePlus"],
    screenSize: ["32 inch", "43 inch", "50 inch", "55 inch"],
    prices: ["15000", "20000", "30000", "40000"],
    type: ["LED", "QLED", "OLED"]
  },

  Trimmers: {
    brand: ["Philips", "Nova", "Bombay Shaving", "Mi"],
    prices: ["300", "500", "800", "1000"],
    type: ["Cordless", "Corded"]
  },

  Watches: {
    brand: ["Boat", "FireBoltt", "Noise", "Fastrack"],
    prices: ["500", "800", "1000", "1500"],
    type: ["Analog", "Digital", "Smartwatch"],
    colors: ["Black", "Blue", "White", "Gray"]
  }
};
